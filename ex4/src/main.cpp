...
bool readFileToMatrix (const std::string &filePath, Matrix &mat)
{
    ...
    long unsigned int length = file.tellg ();
    file.seekg (0, std::ios::beg);
    if (length < mat.get_rows () * mat.get_cols () * sizeof (float))
    {
        return false;
    }
    try
...
{
weights[i] =
Matrix (weights_dims[i]
.rows, weights_dims[i].cols);
biases[i] =
Matrix (bias_dims[i]
.rows, bias_dims[i].cols);
...
" at probability: " << output.probability <<
std::endl;
}
else
{
